            ...
            for (var property in responseJson.data[i]) {
              if (responseJson.data[i].hasOwnProperty(property)) {
                ...
                if (resx.test(responseJson.data[i][property])) {
                  ...
                  responseJson.data[i][property] = ConvertJsonDateToDate(
                    responseJson.data[i][property]
                  )
                }
              }
            }
    ...
    fetch('https://bambai.online/Admin/APIBamBai/updateCart', {
      ...
      .then((responseJson) => {
        if (responseJson.code == 200) {
          ...
          alert('Sản phẩm chưa được cập nhật')
        }
        setactive(true)
      })
      .catch((error) => console.log(error))
        ...
        <View style={styles.container}>
          {UserID == 0 ? (
            ...
            <ScrollView style={{ flex: 1 }}>
              {dataCart.length == 0 ? (
                <Text style={{ fontSize: 30, fontWeight: 'bold', padding: 50 }}>
                  Giỏ hàng trống
                </Text>
              ) : (
                <DataTable style={styles.DataTable}>
                  <DataTable.Header>
                    <DataTable.Title style={styles.textContent}>
                      Ảnh
                    </DataTable.Title>
                    <DataTable.Title numeric style={styles.textContent}>
                      {' '}
                      Giá
                    </DataTable.Title>
                    <DataTable.Title numeric style={styles.textQty}>
                      Số lượng
                    </DataTable.Title>
                  ...
                  {dataCart.map((c) => (
                    <DataTable.Row key={c.CartID}>
                      <DataTable.Cell style={styles.textContent}>
                        {' '}
                        <Image
                          style={styles.cartImg}
                          source={{
                            uri:
                              'https://bambai.online/Content/images/items/' +
                              c.Image,
                          }}
                        />
                      </DataTable.Cell>
                          ...
                          <View style={{ flex: 1, flexDirection: 'row' }}>
                            ...
                            >
                              <Image
                                ...
                                source={{
                                  uri: 'https://image.flaticon.com/icons/png/512/25/25232.png',
                                }}
                              />
                            </TouchableOpacity>
                            <Text
                              style={{ fontSize: 15, width: 10, height: 20 }}
                            >
                              {c.Quantity}
                            </Text>
                            <TouchableOpacity
                            ...
                            >
                              <Image
                                style={{ width: 15, height: 15 }}
                                source={{
                                  uri: 'https://image.flaticon.com/icons/png/512/1828/1828921.png',
                                }}
                              />
                            </TouchableOpacity>
                          </View>
                      ...
                      <DataTable.Cell numeric style={styles.textContent}>
                        ...
                        >
                          <Image
                            style={{ width: 15, height: 15 }}
                            source={{
                              uri: 'https://image.flaticon.com/icons/png/512/1828/1828843.png',
                            }}
                          />
                        </TouchableOpacity>
                      </DataTable.Cell>
                  ...
                  {checkout ? (
                    ...
                    <Button onPress={() => Checkout()} title="Đặt hàng" />
                  )}
                </DataTable>
              )}
            </ScrollView>
