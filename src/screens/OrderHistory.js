                        ...
                        for (var property in responseJson.order[i]) {
                            if (responseJson.order[i].hasOwnProperty(property)) {
                                ...
                                if (resx.test(responseJson.order[i][property])) {
                                    ...
                                    responseJson.order[i][property] = ConvertJsonDateToDate(responseJson.order[i][property]);
                                }
                            }
                        }
                    }
            ...
            {isLoading ? <ActivityIndicator style={{ padding: 100 }} size="large" color="#0000ff" /> :
                <View style={styles.container}>
                    <ScrollView style={{ flex: 1 }}>
                            <Text style={{fontSize:25,fontWeight:'bold',justifyContent:'center'}}>Lịch sử đặt hàng</Text>
                        <Grid  >
                            <Row>
                                <Col style={styles.col} size={32}><Text style={styles.textcol}>ORDER DATE</Text></Col>
                                <Col style={styles.col} size={30}><Text style={styles.textcol}>SHIP DATE{"\n"}(EXPECTED)</Text></Col>
                                <Col style={styles.col} size={20}><Text style={styles.textcol}>STATUS</Text></Col>
                                <Col style={styles.col} size={15}><Text style={styles.textcol}>ACTION</Text></Col>
                            </Row>
                            {dataOrder.map((c) =>
                                <Row key={c.OrderID}>
                                    <Col style={styles.col} size={32}><Text style={styles.textcol}>{c.OrderDate}</Text></Col>
                                    <Col style={styles.col} size={35}><Text style={styles.textcol}>{c.ShipDate}</Text></Col>
                                    <Col style={styles.col} size={20}>{c.status ? <Text style={styles.textcol}>Đang giao</Text> : <Text style={styles.textcol}>Chờ xác nhận</Text>}</Col>
                                    <Col style={styles.col} size={15}>
                                        <TouchableOpacity style={{ marginLeft: 8 }} onPress={() => DetailOrder(c.OrderID)}>
                                            <Image style={{ width: 15, height: 15 }} source={{ uri: 'https://image.flaticon.com/icons/png/512/1/1755.png' }} />
                                        </TouchableOpacity>
                                    </Col>
                                </Row>
                            )}
